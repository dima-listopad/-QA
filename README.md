...
Цель тестирования - Предоставить актуальную информацию о соответсвии продукта его требованиям.
...
Качество - показатель степени соответсвия продукта его требованиям.
...
3. Продуктные требования: 
  - Прямые - это техническое задание(ТЗ), спецификации, макеты, и тд.
  - Косвенные — это что-то не описанное, не задокументированное, на основе предыдущего опыта можно предположить что оно должно так работать.
  - Функциональные (охватывают предполагаемое поведение системы, определяя действия, которые система способна выполнять. Описывается в системной спецификации. В основном влияют на дизайн системы);
  - Нефункциональные (охватывают свойства системы (удобства использования, надежность, масштабируемость) которыми она должна обладать при реализации своего поведения)).
...
*Вопросы:
1. Отличие GET от POST - GET иденпотентный метод (запрос, сделанный один или несколько раз подряд, имеет один и тот же эффект, не изменяющий состояние сервера). GET передает информацию в URL, а POST передает данные в теле запроса. POST не кэшируется.
